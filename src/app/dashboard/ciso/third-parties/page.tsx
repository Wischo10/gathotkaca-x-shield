  ...
  return <>
    ...
    <main className="flex-1 space-y-4 bg-slate-50 p-4 sm:p-6 dark:bg-slate-950">
      ...
      <Panel title="Third-Party Register" action={<span className="text-xs text-slate-400">Manual records only</span>}>
        ...
        {storageAvailable === true && items.length > 0 && <div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-sm"><thead className="border-b text-xs text-slate-500"><tr><th className="py-2">Vendor Code</th><th>Vendor / Service</th><th>Criticality</th><th>Assessed Risk</th><th>Owner</th><th>Assessment</th><th>Last Assessed</th><th>Review</th><th>Action</th></tr></thead><tbody className="divide-y dark:divide-slate-800">{items.map(item => <tr key={item.id}><td className="py-3 font-medium">{item.vendorCode}</td><td>{item.vendorName}<div className="text-xs text-slate-500">{item.providedService}</div></td><td>{item.criticality}</td><td>{item.riskRating ?? "Not Assessed"}</td><td>{item.internalOwner}</td><td>{item.assessmentStatus === "assessed" ? "Assessed" : "Needs Assessment"}</td><td>{item.assessedAt ? new Date(item.assessedAt).toLocaleString() : "Not Assessed"}</td><td>{item.nextReviewDate ?? "N/A"}</td><td>{item.assessmentStatus === "needs_assessment" ? <button type="button" onClick={() => { setSelected(item); setMessage(""); setError(""); }} className="text-brand-blue hover:underline">Complete Assessment</button> : <span className="text-slate-400">Completed</span>}</td></tr>)}</tbody></table></div>}
      </Panel>
