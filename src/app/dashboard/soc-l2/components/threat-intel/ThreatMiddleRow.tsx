...
export function ThreatMiddleRow() {
  ...
  const allIocData = Object.keys(typeCounts)
    .map(key => ({
      name: key,
  ...
  const iocData = allIocData.slice(0, 5);
  ...
  const allIndicators = iocs.map(ioc => ({
  ...
  const indicators = allIndicators.slice(0, 5);
  ...
  return (
    <>
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm flex flex-col h-[320px]">
          ...
          <div className="mt-4 text-right">
            <button onClick={() => setIsIocModalOpen(true)} className="text-xs font-medium text-brand-blue flex items-center gap-1 ml-auto hover:underline">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm flex flex-col h-[320px]">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => setIsIndicatorsModalOpen(true)} className="text-xs font-medium text-brand-blue hover:underline">View All</button>
          </div>
          ...
          <div className="mt-2 text-right">
            <button onClick={() => setIsIndicatorsModalOpen(true)} className="text-xs font-medium text-brand-blue flex items-center gap-1 ml-auto hover:underline">
      ...
      <Modal isOpen={isIocModalOpen} onClose={() => setIsIocModalOpen(false)} title="IOC Summary">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
            <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl flex flex-col items-center justify-center">
              <span className="text-3xl font-bold text-slate-900 dark:text-white">{totalIocs.toLocaleString()}</span>
              <span className="text-sm font-medium text-slate-500">Total IOCs</span>
            </div>
          </div>
          {allIocData.map((item, i) => (
            <div key={i} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm text-slate-500">{totalIocs ? Math.round((item.value / totalIocs) * 100) : 0}%</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <Modal isOpen={isIndicatorsModalOpen} onClose={() => setIsIndicatorsModalOpen(false)} title="All Malicious Indicators">
        {loading ? (
          <div className="flex justify-center p-8"><Loader2 className="w-6 h-6 animate-spin text-brand-blue" /></div>
        ) : allIndicators.length > 0 ? (
          <table className="w-full text-left text-sm">
            <thead className="text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-slate-800/50">
              <tr>
                <th className="py-3 pr-2 font-medium">Indicator</th>
                <th className="py-3 pr-2 font-medium">Type</th>
                <th className="py-3 pr-2 font-medium">Reputation</th>
                <th className="py-3 pr-2 font-medium">First Seen</th>
                <th className="py-3 pr-2 font-medium">Source</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800/30">
              {allIndicators.map((ind, i) => (
                <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 pr-2 font-semibold text-slate-900 dark:text-white">{ind.indicator}</td>
                  <td className="py-3 pr-2 text-slate-500">{ind.type}</td>
                  <td className="py-3 pr-2">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium border ${ind.reputation === 'Malicious' ? 'text-red-700 bg-red-50 border-red-200 dark:text-red-400 dark:bg-red-500/10 dark:border-red-900/50' : 'text-orange-700 bg-orange-50 border-orange-200 dark:text-orange-400 dark:bg-orange-500/10 dark:border-orange-900/50'}`}>
                      {ind.reputation}
                    </span>
                  </td>
                  <td className="py-3 pr-2 text-slate-500 whitespace-nowrap">{ind.firstSeen}</td>
                  <td className="py-3 text-slate-500">{ind.source}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ) : (
          <div className="text-center text-slate-500 py-8">No indicators found</div>
        )}
      </Modal>
    </>
  );
